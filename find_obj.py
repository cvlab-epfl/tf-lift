...
def explore_match_homography(win, img1, img2, kp_pairs, status = None, H = None):
    h1, w1 = img1.shape[:2]
    h2, w2 = img2.shape[:2]
    ...
    cv.imshow(win, vis)
    # cv.imwrite('test.png', vis, [cv.IMWRITE_PNG_COMPRESSION, 0])
...
def explore_match(win, img1, img2, kp_pairs, status = None, H = None):
    ...
    for (x1, y1), (x2, y2), inlier in zip(p1, p2, status):
        if inlier:
            cv.line(vis, (x1, y1), (x2, y2), green, 6)
